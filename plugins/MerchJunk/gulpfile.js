...
function configure(cb) {
  ...
  src("package.json", { ignoreInitial: false })
    .pipe(dest(output))
    .pipe(touch());
  cb();
}
...
function assets(cb) {
  const output = "build";
  return src("src/**/assets/trashcan.png", {
    ignoreInitial: false,
    encoding: false,
  })
    .pipe(dest(output))
...
function attributions(cb) {
  const output = "build";
  return src(["src/**/assets/attribution.md"], { ignoreInitial: false })
    .pipe(dest(output))
    .pipe(touch());
}
//////////////////////////////////
...
function dev(_) {
  ...
  watch(
    ["package.json", "src/**/*"],
    { ignoreInitial: false },
    series(configure, codes, assets, attributions, toc, addons)
  );
}
...
export const build = series(clean, codes, assets, toc, attributions);
